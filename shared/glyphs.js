...
export const GM = { ' ': [0, 22], '1': [9, 43], '4': [0, 64.5], '-': [1, 57], '+': [1, 57], I: [10, 52], M: [0, 70], W: [0, 70], O: [0, 68], Q: [0, 68] };
const ALIAS = { '−': '-' };

let GPATH = null;
const paths = () => {
  if (!GPATH) { GPATH = {}; for (const k in GLYPH) GPATH[k] = new Path2D(GLYPH[k]); }
  return GPATH;
};

export function glyphLayout(str, hPx) {
  const k = hPx / 100, track = 12;
  const chars = [...String(str)].map(ch => ALIAS[ch] || ch).filter(ch => GLYPH[ch] != null);
  const widths = chars.map(ch => { const m = GM[ch] || [0, 62]; return m[1] - m[0]; });
  const total = widths.reduce((a, b) => a + b, 0) + track * Math.max(0, chars.length - 1);
  let cur = -total / 2;
  return chars.map((ch, i) => { const m = GM[ch] || [0, 62]; const x = (cur - m[0]) * k; cur += widths[i] + track; return { ch, x, k }; });
}
// width of a string in pixels at glyph height hPx
export function glyphWidth(str, hPx) {
  const chars = [...String(str)].map(ch => ALIAS[ch] || ch).filter(ch => GLYPH[ch] != null);
  const w = chars.reduce((a, ch) => { const m = GM[ch] || [0, 62]; return a + m[1] - m[0]; }, 0) + 12 * Math.max(0, chars.length - 1);
  return (w * hPx) / 100;
}
export function drawGlyphs(g, str, cx, cy, hPx, sw = 13) {
  const P = paths();
  g.lineCap = 'round'; g.lineJoin = 'round';
  for (const { ch, x, k } of glyphLayout(str, hPx)) {
    g.save(); g.translate(cx + x, cy - 50 * k); g.scale(k, k); g.lineWidth = sw; g.stroke(P[ch]); g.restore();
  }
}
export function glyphSVG(str, cx, cy, hPx, color, sw = 13) {
  return glyphLayout(str, hPx).map(({ ch, x, k }) =>
    `<path d="${GLYPH[ch]}" transform="translate(${(cx + x).toFixed(2)} ${(cy - 50 * k).toFixed(2)}) scale(${k.toFixed(4)})" fill="none" stroke="${color}" stroke-width="${sw}" stroke-linecap="round" stroke-linejoin="round"/>`).join('');
}
// Chapter 1 names
export const drawNumerals = drawGlyphs;
export const numeralSVG = glyphSVG;
export const numH = v => ({ 1: 0.46, 2: 0.42, 3: 0.34 }[String(v).length] || 0.3);
